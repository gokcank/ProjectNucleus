...
| Calendar | `src/features/widgets/calendar/calendar-widget.tsx` | Grid built in pure logic (`calendar-logic.ts`); month and weekday names follow the system locale while the week is pinned to start on Monday, since the locale's own answer is wrong for most users of an en-US desktop outside the US. Re-renders only when the date actually changes, not on every poll |
...
| Brightness | `src/features/widgets/brightness/brightness-widget.tsx` | Volume's two-way control pattern applied to a D-Bus property (GNOME's power daemon, not `/sys/class/backlight`, which needs root to write). Treats "no backlight on this machine" as a state rather than an error, and refuses to drive the screen fully dark |
| Wi-Fi | `src/features/widgets/wifi/wifi-widget.tsx` | Bluetooth's radio-toggle-plus-list shape, over NetworkManager. Collapses the several access points behind one network name, and offers a join only where credentials already exist — an unknown network is listed but visibly inert, since asking for a password is out of scope. `keywords` because "wifi" does not match the title "Wi-Fi" |
...
| Network | `src/features/widgets/network/network-widget.tsx` | Two reads with different costs in one card: connection state comes from D-Bus on mount and `visibilitychange` (Volume's pattern), while the public address leaves the machine and so is fetched once and then only on demand, via `useAsyncAction` |
